...
$insert_query = "INSERT INTO `reg`(`fullName`, `userName`, `email`, `dob`, `gender`, `phoneNumber`, `pass`) VALUES
 ('$reg_fullName','$reg_userName','$reg_email','$reg_dob','$reg_gender','$reg_phoneNumber','$reg_pass')";

$dupe_userName = mysqli_query($conn, "SELECT * FROM `reg` WHERE userName = '$reg_userName'");
$dupe_email = mysqli_query($conn, "SELECT * FROM `reg` WHERE email = '$reg_email'");
...
if (!preg_match($userName_pattern, $reg_userName)) { //userName Match
    ...
    echo "<script>location.href='register.php'</script>";
} else if ($reg_pass !== $reg_conPass) {
    echo "<script>alert('Password & Confirm Password do not match..!!')</script>";
    echo "<script>location.href='register.php'</script>";
} else if (mysqli_num_rows($dupe_userName) > 0) {
